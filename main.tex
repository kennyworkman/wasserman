...
\begin{exercise}
    Let $X_1, X_2 \dots$ be a sequence where $X_n \sim Poisson(\frac{1}{n})$.
    Show $X_n \xrightarrow{P} 0$. If $Y_n = nX_n$, show $Y_n \xrightarrow{P} 0$
\end{exercise}

\begin{proof}
    As $\epsilon \xrightarrow{} 0$, $\P(|X_n| \geq \epsilon)$ is equivalent to
    $\P(X_n > 0) = 1 - e^{-\frac{1}{n}} \xrightarrow{} 0$ because  $X_n \in
    \{0, 1, \dots \}$. Similarly, for sufficiently large $n$, $\P(nX_n
    > \epsilon) = \P(X_n > \frac{\epsilon}{n}) = \P(X_n > 0)$.
\end{proof}

\begin{exercise}
    Suppose we have a computer program consisting of 100 pages of code where
    the number of errors on each page represented by independent $X_i \sim
    Poisson(1)$. Let $Y = \sum_{i=1}^100 X)i$. Approximate $\P(Y < 90)$ using
    the CLT.
\end{exercise}

\begin{proof}
    $\P(Y < 90) = \P(Z < \frac{10(\frac{90}{100} - 1)}{1}) = \Phi(-1) \approx 0.16$.
\end{proof}
