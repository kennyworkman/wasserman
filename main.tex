...
\begin{proof}
    $\operatorname{SE}({\hat{\theta}}) = \sqrt{\Var{\hat{F}_n(b) -
    \hat{F}_n(a)}} = \sqrt{\Var{\hat{F}_n(b)} + \Var{\hat{F}_n(a)}}$. Because
    $\Var{\hat{F}_n(b)} = \frac{F(b)(1-F(b))}{n}$,
    $\operatorname{SE}({\hat{\theta}}) = \sqrt{\frac{F(b)(1-F(b)) + F(a)(1-F(a))}{n}}$. A good estimator for $F(x)$ is the empirical
    distribution function, so $\operatorname{\hat{SE}}({\hat{\theta}}) =
    \sqrt{\frac{\hat{F}_n(b)(1-\hat{F}_n(b)) +
    \hat{F}_n(a)(1-\hat{F}_n(a))}{n}}$.

    We can use DKW, which gives us a confidence interval $\P(\hat{F}_n(x) -
    \epsilon \leq F(x) \leq \hat{F}_n(x) + \epsilon) \leq 2e^{2n\sigma^2}$ for
    fixed $x$, to compute an expanded confidence interval for our difference
    $\hat{F}_n(b) - \hat{F}_n(a)$. This approximate confidence interval is
    $\P((\hat{F}_n(b) - \epsilon) - (\hat{F}_n(a) - \epsilon) \leq \theta
    \leq (\hat{F}_n(b) + \epsilon) - (\hat{F}_n(a) + \epsilon) \leq F(b))$
    https://chatgpt.com/c/695ae21c-5214-832c-9429-2b06dcdf263d
\end{proof}
