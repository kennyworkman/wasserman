...
\begin{exercise}
    ...
    Bernoulli(q)$. Find the estimator, estimated SE and 95 percent CI for $p$ and $p-q$.
\end{exercise}

\begin{proof}
    Because $\E{X_i} = p$, the plug-in estimator $\hat{p_n}$ is appropriate.
    $\hat{p_n} = \bar{X_n}$ with standard error $\sqrt{\frac{\sigma}{n}}$.
    Using the plug-in estimator $n^{-1}\sum_{i=1}^n(X_i - \bar{X_n})^2$ for
    $\sigma$, our estimated standard error is
    $\operatorname{\hat{se}}{(\hat{p_n})} = \frac{n^{-1}\sum_{i=1}^n(X_i -
    \bar{X_n})^2}{\sqrt{n}}$.
    Our 95 percent confidence interval is then
    $(\bar{X_n}-\frac{n^{-1}\sum_{i=1}^n(X_i - \bar{X_n})^2}{\sqrt{n}},
    \bar{X_n}+\frac{n^{-1}\sum_{i=1}^n(X_i - \bar{X_n})^2}{\sqrt{n}})$.

    For $\theta = p - q$, $\hat{\theta} = \hat{p_n} - \hat{q_m} = \bar{X_n} -
    \bar{Y_m}$. $\operatorname{se}{\hat{\theta}} =
    \sqrt{\Var{\hat{\theta}}} = \sqrt{\Var{\hat{p_n} - \hat{q_m}}} =
    \frac{\operatorname{se}{p_n} + \operatorname{se}{q_m}}{\sqrt{n}}$. Our
    estimate is then
    value $\operatorname{\hat{se}}{\hat{\theta_n}} =
    \frac{n^{-1}\sum_{i=1}^n(X_i - \bar{X_n})^2 + m^{-1}\sum_{i=1}^m(Y_i -
    \bar{Y_m})^2}{\sqrt{n}}$.
\end{proof}
