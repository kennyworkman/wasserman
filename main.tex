...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item{If $X_1 \dots X_n$ are independent, $\Var{\sum_i a_i X_i} = \sum a_i^2 \Var{X_i}$}
    \end{itemize}
\end{theorem}
...
\begin{definition}
    ...
    \[S^2_n = \frac{1}{1-n}\sum_i (X_i - \bar{X_n})^2\]
\end{definition}
...
\begin{proof}
    ...
    \[\E{S^2_n} = \E{\frac{1}{n} \sum_i (X_i - \bar{X_n})^2} = \frac{1}{n-1}
        \sum_i \E {X_i^2} - \E{X_n^2} = \frac{1}{n-1}n[(\sigma^2 - \mu^2) -
...
\begin{definition}[The Moment Generating Function]
    $\psi_{X}{t} = \E{e^{tX}} = \int e^{tX} dF_X dx$ is the MGF or Laplace
    Transformation of X. $t$ varies over $\R$
\end{definition}

We will use the MGF to compute the moments of $X$. Assuming $\psi$ is well
defined on the open interval around $t=0$, $\psi'_X(0) =
\frac{d}{dt}\,\E{e^{tX}}|_{t=0} = \E{\frac{d}{dt}e^{tX}}|_{t=0} =
\E{Xe^{tX}}|_{t=0} = \E{X}$.
...
\begin{proof}
    This is equivalent to computing the series $\sum_{x=1}^{\infty}
    \frac{x}{2^x}$.
\end{proof}
